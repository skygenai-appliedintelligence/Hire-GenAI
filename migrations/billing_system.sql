-- =========================
-- NEW BILLING & USAGE TRACKING SYSTEM
-- =========================
...
-- Ledger entry type enum (expanded for specific usage types)
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS cv_parsing_usage (
  ...
  -- Usage details
  file_id                 uuid,  -- Reference to parsed file
  file_size_kb            int NOT NULL DEFAULT 0,
  parse_successful        boolean NOT NULL DEFAULT true,
  
  -- Billing
  unit_price              decimal(10,6) NOT NULL,  -- Price per CV at time of parse
  cost                    decimal(10,2) NOT NULL,  -- Calculated cost (usually unit_price)
  
  -- Metadata
  success_rate            decimal(5,2),  -- Percentage of successfully extracted fields
  metadata                jsonb,  -- Any additional details
  
  created_at              timestamptz NOT NULL DEFAULT now(),
  updated_at              timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_cv_parsing_company ON cv_parsing_usage(company_id);
CREATE INDEX IF NOT EXISTS idx_cv_parsing_job ON cv_parsing_usage(job_id);
CREATE INDEX IF NOT EXISTS idx_cv_parsing_date ON cv_parsing_usage(created_at);

-- =========================
-- INTERVIEW QUESTIONS USAGE
-- =========================
CREATE TABLE IF NOT EXISTS question_generation_usage (
  ...
  question_count          int NOT NULL DEFAULT 0,  -- Number of questions generated
  
  -- Billing
  token_price_per_1k      decimal(10,6) NOT NULL,  -- Price per 1K tokens at time of generation
  cost                    decimal(10,2) NOT NULL,  -- Calculated cost
...
CREATE INDEX IF NOT EXISTS idx_question_gen_company ON question_generation_usage(company_id);
CREATE INDEX IF NOT EXISTS idx_question_gen_job ON question_generation_usage(job_id);
CREATE INDEX IF NOT EXISTS idx_question_gen_date ON question_generation_usage(created_at);

-- =========================
-- VIDEO INTERVIEW USAGE
-- =========================
CREATE TABLE IF NOT EXISTS video_interview_usage (
  id                      uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id              uuid NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  job_id                  uuid NOT NULL REFERENCES jobs(id) ON DELETE CASCADE,
  interview_id            uuid,  -- Reference to interview record
  candidate_id            uuid REFERENCES candidates(id) ON DELETE SET NULL,
  
  -- Usage details
  duration_minutes        decimal(10,2) NOT NULL DEFAULT 0.00,  -- Total interview duration in minutes
  video_quality           text,  -- quality level (SD, HD, etc)
  
  -- Billing
  minute_price            decimal(10,6) NOT NULL,  -- Price per minute at time of interview
  cost                    decimal(10,2) NOT NULL,  -- Calculated cost
  
  -- Metadata
  completed_questions     int NOT NULL DEFAULT 0,  -- Number of questions completed
  total_questions         int NOT NULL DEFAULT 0,  -- Total questions in interview
  metadata                jsonb,  -- Additional details like device info, etc.
  
  created_at              timestamptz NOT NULL DEFAULT now(),
  updated_at              timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_video_interview_company ON video_interview_usage(company_id);
CREATE INDEX IF NOT EXISTS idx_video_interview_job ON video_interview_usage(job_id);
CREATE INDEX IF NOT EXISTS idx_video_interview_date ON video_interview_usage(created_at);

-- =========================
-- JOB USAGE SUMMARY (Aggregated View)
-- =========================
CREATE TABLE IF NOT EXISTS job_usage_summary (
  id                      uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  job_id                  uuid NOT NULL REFERENCES jobs(id) ON DELETE CASCADE,
  company_id              uuid NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  ...
  question_gen_count      int NOT NULL DEFAULT 0,
  total_tokens_used       int NOT NULL DEFAULT 0,
  question_gen_cost       decimal(10,2) NOT NULL DEFAULT 0.00,
  
  interview_count         int NOT NULL DEFAULT 0,
  total_interview_minutes decimal(10,2) NOT NULL DEFAULT 0.00,
  video_interview_cost    decimal(10,2) NOT NULL DEFAULT 0.00,
...
CREATE INDEX IF NOT EXISTS idx_job_usage_summary_company ON job_usage_summary(company_id);
...
CREATE TABLE IF NOT EXISTS usage_ledger (
  ...
  reference_id      uuid,  -- Reference to specific usage record (cv, question, or video)
...
CREATE INDEX IF NOT EXISTS idx_usage_ledger_reference ON usage_ledger(reference_id) WHERE reference_id IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS invoices (
  ...
  -- Metadata
  description           text,
  line_items            jsonb,  -- Array of line items with usage type breakdown
  time_period           jsonb,  -- Start and end date for this invoice
...
VALUES (
  0.50,    -- $0.50 per CV parse
  0.002,   -- $0.002 per 1K tokens
  0.10,    -- $0.10 per minute
...
-- Function to update job usage summary
...
BEGIN
  ...
  ON CONFLICT (job_id) DO UPDATE SET
    cv_parsing_count = v_cv_count,
    cv_parsing_cost = v_cv_cost,
    question_gen_count = v_question_count,
    total_tokens_used = v_token_count,
    question_gen_cost = v_question_cost,
    interview_count = v_interview_count,
    total_interview_minutes = v_interview_minutes,
    video_interview_cost = v_interview_cost,
    total_cost = v_total_cost,
    last_updated = now();
END;
$$ LANGUAGE plpgsql;

-- Triggers to automatically update job summary
CREATE OR REPLACE FUNCTION trigger_update_job_summary()
RETURNS TRIGGER AS $$
BEGIN
  PERFORM update_job_usage_summary(NEW.job_id);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Create triggers for each usage table
DROP TRIGGER IF EXISTS trigger_cv_usage_summary ON cv_parsing_usage;
CREATE TRIGGER trigger_cv_usage_summary
  AFTER INSERT OR UPDATE ON cv_parsing_usage
  FOR EACH ROW
  EXECUTE FUNCTION trigger_update_job_summary();
  
DROP TRIGGER IF EXISTS trigger_question_usage_summary ON question_generation_usage;
CREATE TRIGGER trigger_question_usage_summary
  AFTER INSERT OR UPDATE ON question_generation_usage
  FOR EACH ROW
  EXECUTE FUNCTION trigger_update_job_summary();
  
DROP TRIGGER IF EXISTS trigger_video_usage_summary ON video_interview_usage;
CREATE TRIGGER trigger_video_usage_summary
  AFTER INSERT OR UPDATE ON video_interview_usage
  FOR EACH ROW
  EXECUTE FUNCTION trigger_update_job_summary();
