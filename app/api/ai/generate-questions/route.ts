...
Only provide the questions, no explanations or additional text. Vary phrasing between runs and avoid repeating earlier ideas.
    ...
    try {
      ...
      // Graceful fallback on any AI error (e.g., quota exceeded)
      console.error('AI provider error, falling back to mock questions:', aiErr?.message || aiErr)

      // JD-adapted mock (same logic as no-key branch)
      const text = String(jobDescription || "")
      const words = text
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, " ")
